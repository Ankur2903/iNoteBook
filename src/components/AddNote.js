...
const AddNote = (props) => {
  ...
  return (
    <div>
      ...
      <i class="fa-solid fa-arrow-left mx-2" style={{color: "purple"}}></i>
       <Link aria-current="page" to="/" style={{color: "purple"}}>Home</Link>
      <h2>Create new Notes</h2>
      <p>Add a new note with your info / notes</p>
      <form className='container my-3'>
        <div className="mb-3">
            <label htmlFor="title" className="form-label">Title</label>
            <input type="text" className="form-control" id="title" name = "title" aria-describedby="emailHelp" value={note.title} onChange={onChange} minLength={5} required style={{width:"700px"}}/>
        </div>
        <div className="mb-3">
        ...
        <button disabled={note.title.length<5 || note.description.length<5} type="submit" className="btn btn-primary" onClick={handleClick} style={{color: "white",backgroundColor: "purple", borderRadius: "4px", width: "700px"}}>Add Note</button>
      </form>
      </div>
